...
from . import config
from .knowledge import KnowledgeBase, _bigrams, ascii_token_hit, query_tokens, token_coverage
...
class Researcher:
    ...
    def _filter_results(self, results: list, query: str) -> list:
        ...
        对策：按查询 token 加权覆盖率剔除 0 相关结果，其余按覆盖率降序
        呈现，让模型优先看到最相关的。
        """
        ...
        for r in results:
            ...
            if cov < 0.35:  # 只命中单个泛词（如仅"欧洲"）的弱相关结果一并剔除
                continue
            if not ascii_token_hit(text, tokens):  # 专有名词未命中，泛词再多也算无关
                continue
            overlap = sum(1 for t in q_terms if t in text)
            scored.append((cov, overlap, r))
        scored.sort(key=lambda x: (-x[0], -x[1]))
        return [r for _, _, r in scored[:5]]
    ...
    def _dispatch(self, name: str, args: dict, seen_urls: dict[str, str]) -> tuple[bool, str]:
        ...
        if name == "web_search" and isinstance(args.get("query"), str) and args["query"].strip():
            ...
            self.emit("search", {"query": query, "results": len(kept), "dropped": len(results) - len(kept)})
            with self._lock:
                for r in kept:
                    seen_urls.setdefault(r.url, r.title)
            lines = [f"{i}. {r.title}\n   路径: {r.url}\n   {r.snippet[:120]}" for i, r in enumerate(kept, 1)]
            if not lines:
                return True, ("无相关搜索结果。请换一个更短、更具体的关键词"
                              "（5-15 字、单一主题），不要把多个主题拼在一个搜索词里。")
            return True, "\n".join(lines)
