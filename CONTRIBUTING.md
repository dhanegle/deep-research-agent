...
- **数字不过 LLM**：来自网页/表格的数值、单位、年份、统计口径一律由确定性代码抽取与换算，
  摘要与写作只能引用已抽取的证据，不得让模型转写数字——实测踩过"千美元写成百万美元"的坑。
- **未解决的问题不清零**：自审发现但修订轮没修掉的问题，必须保留到最终产物里
  （报告头部的核验状态与待核验章节），不允许因为一次宽松判定就静默通过。
